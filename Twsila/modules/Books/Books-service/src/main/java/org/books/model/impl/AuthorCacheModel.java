/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package org.books.model.impl;

import com.liferay.petra.lang.HashUtil;
import com.liferay.petra.string.StringBundler;
import com.liferay.portal.kernel.model.CacheModel;

import java.io.Externalizable;
import java.io.IOException;
import java.io.ObjectInput;
import java.io.ObjectOutput;

import java.util.Date;

import org.books.model.Author;

/**
 * The cache model class for representing Author in entity cache.
 *
 * @author Brian Wing Shun Chan
 * @generated
 */
public class AuthorCacheModel implements CacheModel<Author>, Externalizable {

	@Override
	public boolean equals(Object object) {
		if (this == object) {
			return true;
		}

		if (!(object instanceof AuthorCacheModel)) {
			return false;
		}

		AuthorCacheModel authorCacheModel = (AuthorCacheModel)object;

		if (authorId == authorCacheModel.authorId) {
			return true;
		}

		return false;
	}

	@Override
	public int hashCode() {
		return HashUtil.hash(0, authorId);
	}

	@Override
	public String toString() {
		StringBundler sb = new StringBundler(21);

		sb.append("{uuid=");
		sb.append(uuid);
		sb.append(", authorId=");
		sb.append(authorId);
		sb.append(", groupId=");
		sb.append(groupId);
		sb.append(", companyId=");
		sb.append(companyId);
		sb.append(", nom=");
		sb.append(nom);
		sb.append(", prenom=");
		sb.append(prenom);
		sb.append(", email=");
		sb.append(email);
		sb.append(", DateNaissance=");
		sb.append(DateNaissance);
		sb.append(", dateDeces=");
		sb.append(dateDeces);
		sb.append(", autobiographie=");
		sb.append(autobiographie);
		sb.append("}");

		return sb.toString();
	}

	@Override
	public Author toEntityModel() {
		AuthorImpl authorImpl = new AuthorImpl();

		if (uuid == null) {
			authorImpl.setUuid("");
		}
		else {
			authorImpl.setUuid(uuid);
		}

		authorImpl.setAuthorId(authorId);
		authorImpl.setGroupId(groupId);
		authorImpl.setCompanyId(companyId);

		if (nom == null) {
			authorImpl.setNom("");
		}
		else {
			authorImpl.setNom(nom);
		}

		if (prenom == null) {
			authorImpl.setPrenom("");
		}
		else {
			authorImpl.setPrenom(prenom);
		}

		if (email == null) {
			authorImpl.setEmail("");
		}
		else {
			authorImpl.setEmail(email);
		}

		if (DateNaissance == Long.MIN_VALUE) {
			authorImpl.setDateNaissance(null);
		}
		else {
			authorImpl.setDateNaissance(new Date(DateNaissance));
		}

		if (dateDeces == Long.MIN_VALUE) {
			authorImpl.setDateDeces(null);
		}
		else {
			authorImpl.setDateDeces(new Date(dateDeces));
		}

		if (autobiographie == null) {
			authorImpl.setAutobiographie("");
		}
		else {
			authorImpl.setAutobiographie(autobiographie);
		}

		authorImpl.resetOriginalValues();

		return authorImpl;
	}

	@Override
	public void readExternal(ObjectInput objectInput) throws IOException {
		uuid = objectInput.readUTF();

		authorId = objectInput.readLong();

		groupId = objectInput.readLong();

		companyId = objectInput.readLong();
		nom = objectInput.readUTF();
		prenom = objectInput.readUTF();
		email = objectInput.readUTF();
		DateNaissance = objectInput.readLong();
		dateDeces = objectInput.readLong();
		autobiographie = objectInput.readUTF();
	}

	@Override
	public void writeExternal(ObjectOutput objectOutput) throws IOException {
		if (uuid == null) {
			objectOutput.writeUTF("");
		}
		else {
			objectOutput.writeUTF(uuid);
		}

		objectOutput.writeLong(authorId);

		objectOutput.writeLong(groupId);

		objectOutput.writeLong(companyId);

		if (nom == null) {
			objectOutput.writeUTF("");
		}
		else {
			objectOutput.writeUTF(nom);
		}

		if (prenom == null) {
			objectOutput.writeUTF("");
		}
		else {
			objectOutput.writeUTF(prenom);
		}

		if (email == null) {
			objectOutput.writeUTF("");
		}
		else {
			objectOutput.writeUTF(email);
		}

		objectOutput.writeLong(DateNaissance);
		objectOutput.writeLong(dateDeces);

		if (autobiographie == null) {
			objectOutput.writeUTF("");
		}
		else {
			objectOutput.writeUTF(autobiographie);
		}
	}

	public String uuid;
	public long authorId;
	public long groupId;
	public long companyId;
	public String nom;
	public String prenom;
	public String email;
	public long DateNaissance;
	public long dateDeces;
	public String autobiographie;

}